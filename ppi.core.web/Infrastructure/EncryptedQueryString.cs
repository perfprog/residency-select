...
namespace PPI.Core.Web.Infrastructure
{
    public class EncryptedQueryString : Dictionary<string, string>
    {
        ...
        // Must be 8 bytes
        protected byte[] _keyBytes = new byte[] { 0x49, 0x76, 0x61, 0x6e, 0x20, 0x4d, 0x65, 0x64 };

        // Must be at least 8 characters
        protected string _keyString = ConfigurationManager.AppSettings["Encryption__KeyString"];

        // Name for checksum value (unlikely to be used as arguments by user)
        protected string _checksumKey = ConfigurationManager.AppSettings["Encryption__ChecksumKey"];
